...
const Purchases = () => {
  ...
  const updateItem = (id: number, field: string, value: string) => {
    setItems(items.map(item => {
      if (item.id === id) {
        ...
        if (field === 'quantity' || field === 'purchaseRate' || field === 'discountPercent') {
          const qty = parseFloat(updatedItem.quantity) || 0;
          const rate = parseFloat(updatedItem.purchaseRate) || 0;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('Purchase Entry:', { formData, items });
  };

  return (
    <>
        <div className="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
          <h1 className="page-title fw-semibold fs-18 mb-0">Purchase Entry</h1>
        </div>
